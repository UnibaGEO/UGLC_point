...
# Directory containing standardized csv files
...
# Verify if all CSV files are in the dir
if csv_files:
...
# Calculate tile dimension (16x8 grid)
lon_step = (max_lon - min_lon) / 16
lat_step = (max_lat - min_lat) / 8

# Convert the dataframe as GeoDataFrame
gdf = gpd.GeoDataFrame(df_cleaned, geometry=gpd.GeoSeries.from_wkt(df_cleaned['WKT_GEOM']))

# Set the CRS to EPSG:4326 (WGS 84)
gdf.set_crs(epsg=4326, inplace=True)

# Loop on 16x8 grid's row and columns
for i in range(16):
    for j in range(8):
        # Tile frame limit
